...
bool ScaleFactor::check_SameBinning(TGraphAsymmErrors* graph1, TGraphAsymmErrors* graph2){
	...
	if (n1 != n2 ) {
	//std::cout << "Graphs with different number of bins. Check pt binning in data and MC " << std::endl; 
	return false;}
	else {
		haveSameBins = true;
		const int nbins = n1;
...
double ScaleFactor::get_EfficiencyData(double pt, double eta){
	...
	//std::cout << "eta label " << label <<std::endl;
	std::map<std::string, TGraphAsymmErrors*>::iterator it;
	it =  eff_data.find(label);
	if ( it == eff_data.end()) { 
	std::cout << "ERROR in ScaleFactor::get_EfficiencyData(double pt, double eta) from NTupleMaker/src/ScaleFactor.cc : no object corresponding to eta label "<< label << " for data " << std::endl; exit(1);
	}
	...
	// if pt is in range take corresponding efficiency
		...
		if (eff > 1.) {std::cout<< "WARNING in ScaleFactor::get_EfficiencyData(double pt, double eta) from NTupleMaker/src/ScaleFactor.cc: Returned efficiency in data > 1. " << std::endl;} 
		if (eff < 0 ) {std::cout<<"WARNING in ScaleFactor::get_EfficiencyData(double pt, double eta) from NTupleMaker/src/ScaleFactor.cc: Returned negative efficiency in data" <<std::endl;}
	}
	return eff;
}
...
double ScaleFactor::get_EfficiencyMC(double pt, double eta) {
	...
	if ( it == eff_mc.end()) { 
	std::cout << "ERROR in ScaleFactor::get_EfficiencyMC(double pt, double eta) from NTupleMaker/src/ScaleFactor.cc: no object corresponding to eta label "<< label << " for MC " << std::endl; exit(1);
	}
...
double ScaleFactor::get_ScaleFactor(double pt, double eta){
	...
	double SF;
	if ( efficiency_mc == 0){std::cout << "WARNING in ScaleFactor::get_ScaleFactor(double pt, double eta) from NTupleMaker/src/ScaleFactor.cc : MC efficiency = 0. Scale Factor set to 0. "; SF = 0. ;}
